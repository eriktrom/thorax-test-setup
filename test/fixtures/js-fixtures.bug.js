// copied from https://github.com/badunk/js-fixtures/blob/master/test/fixtures-spec.js
// to ensure everything works. The callback spec is broken

// This file is leaking state. Try renaming it to js-fixtures.spec.js
// and uncommenting out the line in test/main.js that requires it
// and you'll notice that load inside of templating-fixtures.spec.js
// now is broken. This seems odd. It's either sinon is still running and returning
// a fake page(don't think so) or the content is being refreshed after the test is
// over... not sure, didn't investigate too deep, we're just using read method

define([
  "sinon/util/event",
  "sinon/util/fake_xml_http_request",
  "sinon/util/fake_server",
  "sinon/stub"
  ], function(){
